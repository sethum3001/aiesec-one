...
export default function Home() {
  const { data: session } = useSession(); // Destructure the session data to access user information.
    ...
    <main className={styles.main}>
      ...
      <Container mb="lg">
        ...
        <Title order={2} mb="xs" mt="lg" style={{ fontWeight: 400 }}>
          Hey {session?.user?.name}!  {/* Dynamically display the user's name from the session data. */}
        </Title>
      ...
      <Button mt="sm" onClick={() => router.push("/login")}>  {/* navigate to the login page. */}
        Explore
      </Button>
    </main>
  );
